...
export async function getWeatherDataByLatLon(position) {
  ...
  const response = await fetch(
    `https://api.openweathermap.org/data/2.5/forecast?lat=${latitude}&lon=${longitude}&appid=${openWeatherApiKey}71&units=imperial`
  );
  const data = await response.json();
  return data;
}
...
export const getGeoLocation = () => {
  ...
  return new Promise((resolve, reject) => {
    ...
    if ("geolocation" in navigator) {
      // resolve();
      // two functions are needed inside: success, error
